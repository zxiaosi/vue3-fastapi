...
const getData = async () => {
  const { data } = await getCourseDetail({ path: PathEnum.elective });
...
const processData = (data: Data[]) => {
  ...
  data.map((item) => {
    ...
    item.teacher_name = item.teacher_name || "无";
...
const spanMethod = computed(() => {
  return getSpanMethod(state.dataList, ["name", "teacher_name"], []);
});
</script>
  ...
  <div>
    ...
    <div class="container">
      <el-table :data="state.dataList" :span-method="spanMethod" border style="width: 100%">
        ...
        <el-table-column prop="teacher_name" label="教师" min-width="160" align="center" />
        <el-table-column prop="course_name" label="课程" min-width="280" align="center" />
        <el-table-column prop="grade" label="成绩" min-width="140" align="center" />
        <el-table-column prop="update_time" label="选课时间" min-width="200" align="center" />
      </el-table>
    </div>
  </div>
</template>
