...
import { PathEnum, type Query, type SelectCourseForm } from "@/types/table";
...
// 表单对象
const formData: SelectCourseForm = reactive({
  id: "",
  grade: "0",
  student_id: "",
  teacher_id: "",
...
const getData = async (currentPage: number = query.currentPage) => {
  let sc = readDatas({ path: PathEnum.elective, pageIndex: currentPage, pageSize: query.pageSize }); // 获取选课表数据
