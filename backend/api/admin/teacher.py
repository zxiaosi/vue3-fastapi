...
from schemas import TeacherOut, TeacherCreate, TeacherUpdate, ResultPlusModel, Relation, ResultModel
...
@router.post("/", response_model=ResultPlusModel[TeacherOut], summary='添加教师信息')
...
@router.put("/{id}", response_model=ResultPlusModel[TeacherOut], summary='通过 id 更新教师信息')
...
@router.delete("/{id}", response_model=ResultPlusModel[TeacherOut], summary='通过 id 删除教师信息')
...
@router.post("/del/", response_class=ORJSONResponse, summary='同时删除多个教师信息')
def delete_departments(*, db: Session = Depends(get_db), idList: list) -> Any:
    crud.teacher.remove_multi(db, id_list=idList)
    return resp_200(msg=f'成功删除多个院系信息.')


@router.get("/relation/", response_model=ResultModel[Relation], summary='获取到 教师表 中的关系字段')
