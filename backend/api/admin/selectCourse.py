...
from schemas import SelectCourseCreate, SelectCourseUpdate, SelectCourseOut, ResultPlusModel, Relation, ResultModel
...
@router.get("/{id}", response_model=ResultPlusModel[SelectCourseOut], summary='根据 id 查询选课信息')
def read_select_course(db: Session = Depends(get_db), id: int = None) -> Any:
    """
    根据 id 查询选课信息
...
@router.post("/", response_model=ResultPlusModel[SelectCourseOut], summary='添加选课信息')
...
@router.put("/{id}", response_model=ResultPlusModel[SelectCourseOut], summary='通过 id 更新选课信息')
...
@router.delete("/{id}", response_model=ResultPlusModel[SelectCourseOut], summary='通过 id 删除选课信息')
...
def delete_departments(*, db: Session = Depends(get_db), idList: list) -> Any:
    crud.selectCourse.remove_multi(db, id_list=idList)
