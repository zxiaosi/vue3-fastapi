...
from schemas import DepartmentUpdate, DepartmentCreate, DepartmentOut, ResultModel, ResultPlusModel, Relation
...
def read_departments(db: Session = Depends(get_db), pageIndex: int = 1, pageSize: int = 10) -> Any:
    """
    ...
    if pageIndex == -1 and pageSize == -1:
        ...
        text = f"查询了第 {pageIndex} 页中的 {pageSize} 个院系信息."
    return resp_200(data=get_departments, msg=text)


@router.get("/{id}", response_model=ResultModel[DepartmentOut], summary='根据 id 查询院系信息')
def read_department(db: Session = Depends(get_db), id: int = None) -> Any:  # 整型,防止用户输入字符串
    """
    根据 id 查询院系信息
...
def create_department(*, db: Session = Depends(get_db), department_in: DepartmentCreate) -> Any:
...
def update_department(*, db: Session = Depends(get_db), id: int, department_in: DepartmentUpdate) -> Any:
    get_department = department.get(db, id=id)
    if not get_department:
        raise IdNotExist(err_desc=f"系统中不存在 id 为 {id} 的院系.")
...
def delete_department(*, db: Session = Depends(get_db), id: int) -> Any:
...
# TODO response_model_exclude 未生效
@router.get("/relation/", response_model=ResultModel[Relation], summary='获取到 院系表 中的关系字段')
def get_department_relation(db: Session = Depends(get_db)) -> Any:
