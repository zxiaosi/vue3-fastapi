...
class Student(Base):
    ...
    major_id = Column(Integer, ForeignKey('major.id', ondelete='CASCADE'), comment='专业编号')

    selectCourses = relationship('SelectCourse')  # 不是字段, 可以通过 student ORM对象引用 selectCourse 表的类集合
